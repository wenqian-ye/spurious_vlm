...
def get_z_prompts_gpt2(dataset_name, question_prompts):
    tokenizer = AutoTokenizer.from_pretrained("gpt2", cache_dir=sys_const.MODEL_CACHE)
    model = GPT2LMHeadModel.from_pretrained("gpt2", pad_token_id=tokenizer.eos_token_id, cache_dir=sys_const.MODEL_CACHE)
...
def get_z_prompts_llama(dataset_name, question_prompts):
    tokenizer = LlamaTokenizer.from_pretrained(sys_const.LLAMA_PATH, cache_dir=sys_const.MODEL_CACHE)
    model = LlamaForCausalLM.from_pretrained(sys_const.LLAMA_PATH, cache_dir=sys_const.MODEL_CACHE)
...
def get_z_prompts_t5(dataset_name, question_prompts):
    model = AutoModelForSeq2SeqLM.from_pretrained('google/flan-t5-large', cache_dir=sys_const.MODEL_CACHE)
    tokenizer = AutoTokenizer.from_pretrained('google/flan-t5-large', cache_dir=sys_const.MODEL_CACHE)
    # question_prompts = text_prompts[dataset_name]['question_openLM']
